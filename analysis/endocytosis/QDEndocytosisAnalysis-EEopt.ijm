macro "QD endocytosis analysis [F12]" {
	...
	for(r=0;r<filelist.length/noImages;r++) {
		...
		if(noSlices==5) {
			...
			selectWindow("ImageStack-0005");
			rename("VesiclesSTEDOnlyImage");
			run("Smooth");	
		} else {
		...
		//Starting the analysis by subtracting the QDSTEDonly from the QDSTED image, and the vesicles STEDonly from the vesicles STED image.
		imageCalculator("Subtract create","QDSTEDImage","QDSTEDOnlyImage");
		rename("QDSTEDSubtractedImage");
		changeValues(-1000, 0, 0);
		imageCalculator("Subtract create","VesiclesSTEDImage","VesiclesSTEDOnlyImage");
		rename("VesiclesSTEDSubtractedImage");
		changeValues(-1000, 0, 0);
		...
		selectWindow("VesiclesSTEDSubtractedImage");
		run("Duplicate...", "title=VesiclesTemp");
		selectWindow("VesiclesTemp");
		run("Smooth");
		run("Smooth");
		run("Smooth");
		run("Smooth");
		...
		imageCalculator("Multiply create", "VesiclesTemp","VesiclesSTEDSubtractedImage");
		rename("VesicleSTEDImage2");
		run("Smooth");
		run("Smooth");
		run("Smooth");
		run("Smooth");
